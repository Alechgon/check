...
let CFG = loadCfg();
let ALL = [];               // casos derivados a este técnico (míos)
let TODOS = [];             // casos derivados a cualquier técnico (vista Todos)
...
function updateGpsPills() {
  $$('.gps-pill').forEach(p => {
    ...
    if (txt) txt.textContent = ok ? ('Ubicación lista · ±' + Math.round(lastGps.acc) + 'm') : (lastGps ? 'Actualizando ubicación…' : 'Ubicación opcional');
  });
  if (typeof paintGpsMini === 'function') paintGpsMini();
}
...
function renderGenerales(keep) {
  ...
  const base = activos(genFiltro === 'mios' ? ALL.filter(r => norm(r.derivadoA) === norm(CFG.tecnico)) : (TODOS.length ? TODOS : ALL));
  const pend = base.filter(r => estadoDe(r).k === 'pend').length;
  const fin = base.filter(r => estadoDe(r).k === 'fin').length;
  const emerg = base.filter(r => esEmergencia(r) && estadoDe(r).k === 'pend').length;
  content.innerHTML = `<div class="screen">
    <div class="mkpis">
      <div class="mkpi o"><div class="bar"></div><div class="n" id="kpiPend">${pend}</div><div class="l">Pendientes</div></div>
      <div class="mkpi"><div class="bar"></div><div class="n" id="kpiEm">${emerg}</div><div class="l">Emergencias</div></div>
      <div class="mkpi g"><div class="bar"></div><div class="n" id="kpiFin">${fin}</div><div class="l">Cerrados</div></div>
    </div>
...
function paintGenList() {
  ...
  // Todos = casos derivados a cualquier técnico; Míos = solo los míos
  let list = activos(genFiltro === 'mios' ? ALL : (TODOS.length ? TODOS : ALL));
...
function openCase(r, from) {
  ...
  content.innerHTML = `<div class="screen"><div style="flex:1;overflow-y:auto;display:flex;flex-direction:column">
    ...
    <div class="state-box">
      ...
      ${cerrado ? `<div class="state-btn sel-sol" style="cursor:default">✓ Solucionado</div>${r.fechaSolucionado ? `<p class="note">Cerrado el ${esc(r.fechaSolucionado)}${r.tiempoResolucion ? ' · ' + esc(r.tiempoResolucion) : ''}${r.solucionadoPor ? ' · por ' + esc(r.solucionadoPor) : ''}.</p>` : ''}${r.comentarioTecnico ? `<p class="note" style="margin-top:4px">📝 ${esc(r.comentarioTecnico)}</p>` : ''}` :
      ...
      ${r.derivadoA && norm(r.derivadoA) !== norm(CFG.tecnico) ? `<p class="note" style="margin-top:10px;color:var(--orange-d)">Derivado a ${esc(r.derivadoA)}. Si lo cierras tú, quedará registrado que lo solucionaste.</p>` : ''}`}
    </div>
  </div></div>`;
...
function openCierre(r) {
  cierreState = { usoMateriales: null, materiales: [], verificadores: [], uploading: 0, comentario: '' };
  ...
  ov.innerHTML = `<div class="modal">
    ...
    <div class="modal-foot" style="flex-direction:column;gap:8px">
      <div id="cierreHint" class="note" style="width:100%;text-align:center;margin:0;color:var(--orange-d);display:none"></div>
      <div style="display:flex;gap:10px;width:100%">
        <button class="btn ghost" id="cierreCancel" style="flex:0 0 auto">Cancelar</button>
        <button class="btn green" id="cierreOk">Marcar solucionado</button>
      </div>
    </div>
  </div>`;
...
function paintCierre() {
  ...
  body.innerHTML = `
    <div class="field-block" style="margin-bottom:14px">
      <label class="fld">Indique — ¿qué se hizo?</label>
      <textarea id="cierreComent" placeholder="Describe el trabajo realizado, la solución aplicada…" style="min-height:80px">${esc(s.comentario||'')}</textarea>
    </div>
    <div class="verif-lbl" style="margin-top:0">Verificadores del trabajo</div>
    ...
    <div class="gps-mini" id="gpsMini"></div>
    <div class="verif-lbl">¿Usó materiales?</div>
    <div class="yesno">
    ...
    <div id="matZone"></div>
  `;
  $('#cierreComent').oninput = e => { cierreState.comentario = e.target.value; updateCierreOk(); };
  $('#matSi').onclick = () => { cierreState.usoMateriales = true; paintCierre(); updateCierreOk(); };
  $('#matNo').onclick = () => { cierreState.usoMateriales = false; cierreState.materiales = []; paintCierre(); updateCierreOk(); };
  if (cierreState.usoMateriales === true) paintMatZone();
  bindUploader();
  paintGpsMini();
  updateCierreOk();
}
function paintGpsMini() {
  const el = $('#gpsMini'); if (!el) return;
  const ok = gpsFresh();
  el.className = 'gps-mini ' + (ok ? 'ok' : (lastGps ? 'wait' : 'off'));
  el.innerHTML = ok
    ? `<span class="gd"></span>Ubicación lista · ±${Math.round(lastGps.acc)}m`
    : (lastGps ? `<span class="gd"></span>Actualizando ubicación…` : `<span class="gd"></span>Ubicación opcional · <b id="gpsAsk" style="text-decoration:underline;cursor:pointer">activar</b>`);
  const ask = $('#gpsAsk'); if (ask) ask.onclick = () => { startGpsWatch(); toast('Pídele permiso de ubicación al navegador si aparece', 2600); };
}
...
function updateCierreOk() {
  ...
  const hint = $('#cierreHint');
  const s = cierreState;
  // razones por las que aún no se puede cerrar (en orden de prioridad)
  let motivo = '';
  if (s.uploading > 0) motivo = 'Espera a que terminen de subir los archivos…';
  else if (s.verificadores.some(v => v.error)) motivo = 'Hay archivos con error: reintenta (toca el recuadro) o elimínalos.';
  else if (s.usoMateriales === null) motivo = 'Indica si usaste materiales (Sí / No).';
  else if (s.usoMateriales === true && s.materiales.length === 0) motivo = 'Agrega al menos un material o marca “No”.';
  const ready = !motivo;
  btn.disabled = !ready;
...
async function confirmarCierre(r, close) {
  ...
  if (s.usoMateriales === null) { toast('Indica si usaste materiales'); return; }
  if (s.usoMateriales === true && !s.materiales.length) { toast('Agrega al menos un material o marca No'); return; }
  ...
  const res = await postAction({
    ...
    derivadoA: r.derivadoA || CFG.tecnico,     // el técnico al que estaba derivado
    solucionadoPor: CFG.tecnico,               // quién lo cerró (puede ser otro)
    tecnico: CFG.tecnico,
  ...
  if (res && res.ok) {
    ...
    [ALL, TODOS].forEach(lst => { const a = lst.find(x => x.id === r.id && x.encargado === r.encargado); if (a) { a.visado = 'SOLUCIONADO'; a.fechaSolucionado = r.fechaSolucionado; a.verificadoresTecnico = verifStr; } });

...
function estadoDe(r) {
  ...
  return { k: 'pend', t: 'Pendiente' };  // para el técnico: derivado = pendiente hasta que cierre
}
...
async function fetchMine() {
  ...
  try {
    const res = await fetch(CFG.sheetUrl + '?tecnico=' + encodeURIComponent(CFG.tecnico) + '&t=' + Date.now());
...
function renderConfig() {
  ...
  btnNext.onclick = () => {
    ...
    toast('Guardado · ' + nombreReal);
    refreshData(false).then(() => { renderHome(); startPolling(); });
  };
  bindTecSearch();
}
...
function caseCardHTML(r) {
  ...
  return `<div class="case ${em ? 'em' : ''}" data-id="${esc(r.id)}" data-enc="${esc(r.encargado)}">
    ...
    <div class="cbody">
      ...
      <div class="cmeta"><span>${em ? '🚨 ' : ''}${esc(r.categoria || '')}</span><span>${esc(r.fecha || '')}</span><span>👷 ${esc(r.encargado || '')}</span><span class="cstate ${em && st.k !== 'fin' ? 'em' : st.k}">${st.t}</span></div>
    </div>
...
function renderGenerales(keep) {
  ...
  const list = activos(ALL).filter(r => estadoDe(r).k === 'pend').sort((a, b) => tsOf(b) - tsOf(a));
  const emerg = list.filter(esEmergencia).length;
  content.innerHTML = `<div class="screen">
    <div class="mkpis">
      <div class="mkpi o"><div class="bar"></div><div class="n">${list.length}</div><div class="l">Pendientes</div></div>
    ...
    <div class="verif-lbl">Casos asignados a ti</div>
    <div class="caselist" id="genList">${list.length ? list.map(caseCardHTML).join('') : `<div class="empty"><div class="ic">✅</div><p>No tienes casos pendientes.</p></div>`}</div>
  </div>`;
  bindCaseCards($('#genList'), list);
}
